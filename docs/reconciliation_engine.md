...
> **Core Purpose**: Multi-pass transaction reconciliation, primary statement pool separation, four-status taxonomy v2, Groq LLM integration, and symmetric mirror-pair deduplication.
...
```
  ...
  │ Generates audit reports & updates QA agent DB            │
  └────────────────────────────┬─────────────────────────────┘
```
...
Groq LLM (`openai/gpt-oss-120b`, `qwen/qwen3.6-27b`, `openai/gpt-oss-20b`) is invoked **on-demand** from the UI (e.g. "Smart AI Import", "Organize Table by AI", and candidate review modal `/api/transactions/llm-smart-match`) using multi-key failover (`GROQ_API_KEY`, `GROQ_API_KEY1`, `GROQ_API_KEY2`).
...
| `reconciler/reconcile.py` | `reconcile_all_statements()` | Main multi-statement reconciliation engine. |
